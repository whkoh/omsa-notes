...
- Margin support vectors ($\xi_i=0$) :: we have $Y_i(\beta_0+\beta^T x_i) =1 ~ \text{i.e. } \beta_0+\beta^Tx_i=Y_i\in\pm1$
- Non-margin support vectors ($0<\xi_i\le1$) :: we have $Y_i(\beta_0+\beta^Tx_i)=1-\xi_i\ge0 ~ \text{i.e. }Y_i=\text{sign }(\beta_0+\beta^Tx_i)$
- Misclassification errors ($\xi_i\gt1$) :: since
  $Y_i(\beta_0+\beta^T x_i)=1-\xi_i <0 ~ \text{i.e. } Y_i\ne \text{sign}(\beta_0+\beta^Tx_i)$
*** Linearly non-separable scenarios
[[./img/non-sep-types.png]]
...
- Perfect classification for training data is replaced by {{{hl(misclassification error)}}} for training data $\sum^n_{i=1}(\xi_i > 1)$
- Mathematical formulation for SVM classifier in the {{{hl(linearly non-separable)}}} scenario needs to adjust the training error
- Optimization algorithms are similar.
** Optimization problem for linearly non-separable
11.2.2
*** Three criteria in SVM
...
Y_i(\beta_0+\beta^Tx_i)-1+\xi_i\ge0, ~ , \xi_i \ge 0, ~ \forall i=1,2,...,n
$$
- Unfortunately, this is difficult to optimize as it is {{{hl(non-convex)}}}
- We can motivate another useful formulation
*** Key idea of new formulation
New formulation of SVM for linearly non-separable:
- (P2): find the linear function $h(x)=\beta_0+\beta^Tx$ and $(\xi_1,...,\xi_n)$ that minimizes the objective function
  $$
  ...
  Y_i(\beta_0+\beta^Tx_i)-1+\xi_i\ge0, ~ , \xi_i \ge 0, ~ \forall i=1,2,...,n
  $$
  This is a constrained quadratic optimization data that is solvable
*** Constrained optimization
- By {{{hl(Lagrange multipliers)}}}, we reformulate the constrained primal problem into the unconstrained dual space with the objective function
	$$
	...
	$$
  where $\alpha_i \ge 0, \delta_i \ge 0$ are the **Lagrange multipliers**
- The optimal solution gives the SVM classifier in the linearly non-separable scenario
** SVM for linearly non-separable
11.2.3
...
*** Constrained optimization
- By {{{hl(Lagrange multipliers)}}}, we reformulate the constrained primal problem into unconstrained dual space with the objective function
	$$
	g=\frac{1} {2} \| \beta\|^{2}+C \sum_{i=1}^{n} \xi_{i}-\sum_{i=1}^{n} \alpha_{i} \big[ Y_{i} \big( \beta_{0}+\beta^{T} x_{i} \big)-1+\xi_{i} \big]-\sum_{i=1}^{n} \delta_{i} \xi_{i}
	$$
  where $\alpha_i\ge0, \delta_i\ge0$ are the Lagrange multipliers
*** Derivatives on $(\beta_0,\beta)$
$$
g=\frac{1} {2} \| \beta\|^{2}+C \sum_{i=1}^{n} \xi_{i}-\sum_{i=1}^{n} \alpha_{i} \big[ Y_{i} \big( \beta_{0}+\beta^{T} x_{i} \big)-1+\xi_{i} \big]-\sum_{i=1}^{n} \delta_{i} \xi_{i}
$$
- Taking derivatives w.r.t. $(\beta_0,\beta)$ yields
	$$
...
- Thus a local minimum must satisfy
	...
	$$
  This is the same as those in the linearly separable scenario!
*** Derivatives on $\xi_i$
$$
g=\frac{1} {2} \| \beta\|^{2}+C \sum_{i=1}^{n} \xi_{i}-\sum_{i=1}^{n} \alpha_{i} \big[ Y_{i} \big( \beta_{0}+\beta^{T} x_{i} \big)-1+\xi_{i} \big]-\sum_{i=1}^{n} \delta_{i} \xi_{i}
$$
- Taking derivatives w.r.t. $\xi_i$ yields
  $$
  \frac{\partial g}{\partial\xi_i} = C-\alpha_i-\delta_i
  $$
- Thus a local optimum satisfies $\alpha_i + \delta_i = C \forall i=1,2,...,n$
- This leads to a crucial new constraint on $\alpha_i$ in the linearly non-separable scenario:
  $$
...
g=\frac{1} {2} \| \beta\|^{2}+C \sum_{i=1}^{n} \xi_{i}-\sum_{i=1}^{n} \alpha_{i} \big[ Y_{i} \big( \beta_{0}+\beta^{T} x_{i} \big)-1+\xi_{i} \big]-\sum_{i=1}^{n} \delta_{i} \xi_{i}
$$
- Putting the relation $\delta_i = C-\alpha_i \forall i=1,...,n$, the objective function is reduced to the form:
	$$
	g=\frac{1} {2} \| \beta\|^{2}-\sum_{i=1}^{n} \alpha_{i} \big[ Y_{i} \big( \beta_{0}+\beta^{T} x_{i} \big)-1 \big]
	$$
  which is the same as before in the linearly separable scenario
...
- Since most $\alpha_i$ are 0, the SVM classifier can be rewritten as
  ...
  if we eliminate non-zero terms
